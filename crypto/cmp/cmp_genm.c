                                  ...
                                  X509 **newWithOld, X509 **oldWithNew)
{
    ...
    *newWithNew = NULL;
    ...
    if ((oldWithOld_copy = X509_dup(oldWithOld)) == NULL && oldWithOld != NULL)
        goto end;
    if (!verify_ss_cert_trans(ctx, oldWithOld_copy, *newWithOld,
                              *newWithNew, "newWithNew")) {
        ERR_raise(ERR_LIB_CMP, CMP_R_INVALID_ROOTCAKEYUPDATE);
        goto end;
    }
    if (oldWithOld != NULL && *oldWithNew != NULL
        && !verify_ss_cert_trans(ctx, *newWithNew, *oldWithNew,
    ...
    if (!X509_up_ref(*newWithNew))
        goto end;
    if (newWithOld != NULL && *newWithOld != NULL && !X509_up_ref(*newWithOld))
        goto free;
    if (oldWithNew == NULL || *oldWithNew == NULL || X509_up_ref(*oldWithNew)) {
        res = 1;
        goto end;
    }
    if (newWithOld != NULL)
